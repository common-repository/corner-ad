	...
	function codepeople_add_promote_banner( $wp_admin_bar ) {
		...
		if ( isset( $_POST['codepeople_promote_banner_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['codepeople_promote_banner_nonce'] ) ), __FILE__ ) ) {
			if (
				...
				set_transient( 'codepeople_promote_banner_' . sanitize_text_field( wp_unslash( $_POST['codepeople_promote_banner_plugin'] ) ), -1, 0 );
				if (
					...
					print '<script>document.location.href="' . esc_attr( str_replace( '&amp;', '&', esc_js( $codepeople_promote_banner_plugins[ sanitize_text_field( wp_unslash( $_POST['codepeople_promote_banner_plugin'] ) ) ]['plugin_url'] ) ) ) . '";</script>';
				}
			}
		}
