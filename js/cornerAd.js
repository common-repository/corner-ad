(function(){
		...
		visitUrl:	function( settings )
			{
				...
				if(settings['ga'])
				{
					if('gtag' in window)
					{
                        gtag(
                            'event',
                            (('ga_action' in settings && !/^\s*$/.test(settings['ga_action'])) ? settings['ga_action'] : 'click'),
                            {
                                'send_to': settings['ga'],
                                'event_category': (('ga_category' in settings && !/^\s*$/.test(settings['ga_category'])) ? settings['ga_category'] : 'Corner Ad'),
                                'event_label': (('ga_label' in settings && !/^\s*$/.test(settings['ga_label'])) ? settings['ga_label'] : settings['adUrl']),
                                'event_callback': function(){
                                    if( !me.isEmpty( settings[ 'adUrl' ] ) )
                                    {
                                        window.open(
                                            settings['adUrl'],
                                            settings['target']
	...
	window[ 'printCornerAd' ] = function ( settings )
	{
		...
		function expandLeaf()
		{
			...
			var min = Math.min(cornerContainer.parent().width(),cornerContainer.parent().height()),
				containerSize = Math.min(500,min),
				zoom = containerSize/500;

			cornerContainer.css({'width':containerSize,'height':containerSize});
			cornerContainer.find('svg').css({'transform': 'scale('+zoom+')', 'transform-origin': origin});
			...
			setTimeout(
				function()
				{
					...
					backgroundFillBig.stop().animate( { path: pathSmall}, 400, 'linear',function(){
						...
						cornerContainer.find('svg').css({'transform': 'scale(1)', 'transform-origin': origin});
        ...
        // Google Analytics
        if('ga' in settings && settings['ga'] != '')
        {
            if(!('gtag' in window))
            {
                var gtagScript = document.createElement("script");
                gtagScript.type = "text/javascript";
                gtagScript.setAttribute("src", "https://www.googletagmanager.com/gtag/js?id="+settings['ga']);
                ...
                gtagScript.onload = function() {
                    ...
                    gtag('js', new Date());
                    gtag('config', settings['ga'], {'send_page_view': false});
                };
            }
        }
		...
		if( !tools.isEmpty( settings[ 'imgPath' ] ) )
		{
			...
			var _pattern_s = cornerContainer.find('pattern:eq(0)'),
				_image_s = _pattern_s.find('image'),
				_pattern = cornerContainer.find('pattern:eq(1)'),
				_image = _pattern.find('image');

			_image_s.on('load', function(){
				(function _untilWidth()
				{
					var _w = _image_s.attr('width');
					if('undefined' == typeof _w)
					{
						setTimeout(_untilWidth,100);
						return;
					}
					_pattern_s.attr('width',100);
					_pattern_s.attr('height',100);
					_image_s.attr('width',100);
					_image_s.attr('height',100);
				})();
			});
			...
			_image.on('load', function(){
				(function _untilWidth()
				{
					var _w = _image.attr('width');
					if('undefined' == typeof _w)
					{
						setTimeout(_untilWidth,100);
						return;
					}
					_pattern.attr('width',400);
					_pattern.attr('height',400);
					_image.attr('width',400);
					_image.attr('height',400);
				})();
			});
